...
VALUES('MEM04','1Q2WEE4R','신짱구','960606-1234567');
...
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER /*CONSTRAINT USER_NNO_PK PRIMARY KEY*/,
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
	--> 다름 테이블이 제공하는 컬럼에는 PK 또는 UNIQUE 제약 조건이 설정 되어야 한다.
...
-- ORA-02291: 무결성 제약조건(KH_NMJ.GRADE_CODE_FK1)이 위배되었습니다- 
-- 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_NMJ.GRADE_CODE_FK1)이 위배되었습니다-
-- 자식 레코드가 발견되었습니다
...
AS
SELECT EMP_NAME, NVL(DEPT_TITLE,'소속없음') DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
...
REFERENCES DEPARTMENT; --DEPARMENT 테이블 PK 컬럼 참조
