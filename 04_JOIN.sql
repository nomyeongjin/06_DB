...
-- 부서명이 '해외영업2부'인 사원의
-- 사번(EMP_ID), 이름(EMP_NAME), 부서명(DEPT_TITLE)을 사번 오름차순으로 조회
...
--> 모든 경우의 수를 보고 싶을때 빼고는 직접 작성하는 경우가 없음
  --> 자연 조인 실패 결과로 교차조인 결과가 출력됨
		--(교차 조인 결과가 보이면 -> 아....자연조인 잘못썼네)
...
-- EMPLOYEE, DEPATMENT, LOCATION 3개 조인
...
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
JOIN LOCATION  ON (LOCATION_ID = LOCAL_CODE) -- EMPLOYEE에 없는 컬럼명으로 JOIN 하려고 해서 오류
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
-- ANSI
SELECT DEPT_CODE 사번, EMP_NAME 이름, DEPT_TITLE 부서명, LOCAL_NAME 근무지역명, SALARY 급여
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- SELF JOIN 사용
SELECT I.EMP_NAME 사원명, E.DEPT_CODE 부서코드, E.EMP_NAME 동료이름
FROM EMPLOYEE I JOIN EMPLOYEE E ON(E.DEPT_CODE=I.DEPT_CODE)
