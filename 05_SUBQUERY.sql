...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE)
WHERE (DEPT_CODE, JOB_CODE) = ( --D9	J2
	SELECT DEPT_CODE, JOB_CODE
	FROM EMPLOYEE
	WHERE EMP_NAME = '노옹철'
)
...
-- 2) 직급별 평균 급여
SELECT JOB_CODE,TRUNC(AVG(SALARY),-4) "평균 급여"
FROM EMPLOYEE
GROUP BY JOB_CODE;
 ...
 * 2) 서브쿼리에서 전달받은 값을 이용해서 SELECT 수행
...
FROM(
	...
	SELECT EMP_NAME, SALARY
	FROM EMPLOYEE 
	ORDER BY SALARY DESC --> 조회 결과가 메인쿼리의 테이블 역할
)
WHERE ROWNUM <= 5;
...
SELECT FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(19||SUBSTR(EMP_NO,1,6)))/12) 나이,
CASE
...
SELECT EMP_ID , EMP_NAME , JOB_NAME,
	FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(19||SUBSTR(EMP_NO,1,6)))/12) 나이,
	TO_CHAR(CASE
		WHEN BONUS IS NOT NULL THEN (SALARY+SALARY*BONUS)*12
		ELSE SALARY 
	END,'L999,999,999') "보너스 포함 연봉"

FROM EMPLOYEE MAIN
LEFT JOIN JOB J ON(MAIN.JOB_CODE=J.JOB_CODE)
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(19||SUBSTR(EMP_NO,1,6)))/12) =
(
	SELECT MIN(FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(19||SUBSTR(EMP_NO,1,6)))/12))
	FROM EMPLOYEE SUB
	WHERE SUB.JOB_CODE = MAIN.JOB_CODE
)
ORDER BY FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(19||SUBSTR(EMP_NO,1,6)))/12) DESC;
