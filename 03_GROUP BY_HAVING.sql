...
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE "부서 코드", SUM(SALARY)"부서 별 급여의 합계", ROUND(AVG(SALARY))"부서 별 급여의 평균" , COUNT(*) "인원 수"
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
/* 7행 */
SELECT DEPT_CODE , COUNT(BONUS) 수
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
-- 인원수로 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','여') 성별, FLOOR(AVG(SALARY)) "급여 평균", SUM(SALARY) "급여 합계", COUNT(*) "인원 수"  
FROM EMPLOYEE
