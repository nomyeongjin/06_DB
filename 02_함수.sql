...
FROM DUAL; -- 3 (INDEX X, 몇 번째에 위치해 있는가)

-- 문자열에서 5번부터 검색시작해서 처음 찾은 'B'의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA','B',5) 
FROM DUAL; -- 9


-- 문자열에서 5번부터 검색시작해서 2번째로 찾은 'B'의 위치 조회
...
-- 문자열 공백 제거
SELECT '    기  준   ',
			TRIM(LEADING  ' ' FROM '    기  준   '), -- 앞쪽 공백제거
			TRIM(TRAILING ' ' FROM '    기  준   '), -- 뒷쪽 공백제거
			TRIM(BOTH     ' ' FROM '    기  준   ')  -- 양쪽 공백제거
FROM DUAL;
...
SELECT EMP_ID, EMP_NAME , HIRE_DATE , CEIL(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/12) || '년차' n년차
FROM EMPLOYEE;
...
-- 다음 달 첫 번째, 마지막 날짜
SELECT 
	LAST_DAY(SYSDATE)+1, -- 3월 1일 
	LAST_DAY(ADD_MONTHS(SYSDATE,1)) -- 3월 31일
FROM DUAL;
...
SELECT 1234, TO_CHAR(1234) FROM DUAL; -- 데이터 타입 변경 완료

SELECT 1234, TO_CHAR(1234,'99999999')FROM DUAL; -- 오른쪽 정렬, 빈칸 4칸

SELECT 1234, TO_CHAR(1234,'00000000')FROM DUAL; -- 오른쪽 정렬, 4칸에 0 추가
...
-- 모든 사원의 연봉 조회
SELECT EMP_NAME , TO_CHAR(SALARY *12,'L999,999,999') || '원' 연봉
FROM EMPLOYEE;
...
SELECT SYSDATE,TO_CHAR(SYSDATE,'YYYY/MM/DD (DY) PM HH:MI:SS') FROM DUAL;

-- /, (), :, - 는 일반적으로 날짜 표기시 사용하는 기호
--> 패턴으로 인식되어 오류가 발생하지 않음

-- 년, 월, 일 같은 한글 또는 날짜와 관련없는 문자는 패턴 x
...
SELECT TO_DATE('2024-03-04') FROM DUAL;
-- 문자열이 날짜를 표현하는 형식이면 포맷 지정없이 바로 변경 가능

SELECT TO_DATE('04-03-2024', 'DD-MM-YYYY') FROM DUAL;


SELECT TO_DATE('2월 27일 화요일 12시 24분', 'MM"월" DD"일" DAY HH"시" MI"분"') FROM DUAL;
...
-- <NULL 처리 연사> : IS NULL / IS NOT NULL
...
/* NULL이랑 산술 연상 수행 시 결과는 무조건 NULL */
...
-- (조회 결과가 사각형 형태의 테이블 모양이면 조회 가능)
SELECT SUM(SALARY) 합계, FLOOR(AVG(SALARY))평균  FROM EMPLOYEE;
-- SELECT SALARY 합계, FLOOR(AVG(SALARY))평균  FROM EMPLOYEE;
-- 00937: 단일 그룹의 그룹 함수가 아닙니다
...
-- EMPLOYEE 테이블 전체 행의 갯수
SELECT COUNT(*) FROM EMPLOYEE e ;

-- EMPLOIYEE 테이블에서 부서코드가 'D5'인 사원의 수
SELECT COUNT(*)
FROM EMPLOYEE
WHERE DEPT_CODE ='D5';
